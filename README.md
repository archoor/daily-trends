...
- **导航**：顶部 Logo「Daily Trends」、Home、各数据源、右侧语言切换；当前页紫色高亮。
- **首页**：标题区 + 数据源卡片网格，悬停有边框与阴影反馈。
- **列表页**：标题区 + 数据表格；排行前三名金/银/铜奖牌，增长数据绿色高亮。（数据源切换仅通过顶部导航）
- **详情页**：返回链接、标题、描述列表与标签 pill 样式统一。
- **手机端适配（≤768px）**：根据视口宽度自动切换为手机布局。导航折叠为汉堡菜单（点击展开/收起）；列表页表格变为卡片式布局，每行一条卡片、字段带标签；首页数据源卡片单列排布；详情页键值对改为单列；日期选择条支持横向滑动。无需单独「检测手机」——使用 CSS 媒体查询，同一套 HTML 在桌面与手机上自动呈现对应布局。
